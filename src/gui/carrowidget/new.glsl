...
void main (void){
    ...
    vec4 v_light = normalize(lightPos - vertex_pos);
    float normal_light_dot = dot(v_light, v_normal);

    // set ambient, diffuse, specular color
    vec3 Ia = Ka * light_intensity / lightPosLen;
    vec3 Id = Kd * max(normal_light_dot, 0.0);
    vec3 Is = Ks * pow(max(normal_light_dot, 0.0), Ns);

    float theta = clamp(normal_light_dot, 0.0, 1.0);
    gl_FragColor = vec4(theta * Id + 0.2 * Is /* + 0.00005 * Ia */, Tr);
}
